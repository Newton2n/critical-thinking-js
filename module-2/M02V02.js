...
const toGetTopElectronics = (rawData) => {
  const electronicArr = rawApiData.filter((item) => {
    return item.category === "Electronics";
  });

  const sortElement = electronicArr.sort((a, b) => a.rating - b.rating);

  const topThreeItem = sortElement.slice(-3);
  const modifiedArr = topThreeItem.map((item) => {
    return { name: item.productName };
  });
  return modifiedArr;
};

console.log(toGetTopElectronics(rawApiData));
