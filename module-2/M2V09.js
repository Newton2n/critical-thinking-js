...
// solve
const postByUserId = posts.reduce((acc, currentValue) => {
...
// console.log(postByUserId["101"]);

const postByUser = users.map((user) => {
  const obj = { ...user, post: postByUserId[user.id] };
  return obj;
});

console.log(JSON.stringify(postByUser))
